<?php 
	...
	include ("conexao.php");
	$dataInicial=date("y/m/d",strtotime($_POST['data_inicio']));
	...
	$sql="SELECT pr.nome AS nomepre, orc.idOrcamentos, ct.categoria, sv.servico, cli.nome AS nomecli, orc.data, orc.data_expiracao, orc.valor 
	...
	WHERE orc.data >= '$dataInicial' AND orc.data <= '$dataFinal' ORDER BY orc.data";
    $resultado = mysqli_query($con,$sql) or die ("Erro ao retornar dados");


    $linha="";
    while ($registro=mysqli_fetch_array($resultado)){
        $linha.="<tr><td>".date("d/m/y",strtotime($registro['data']))."</td>";
		$linha.="<td>".$registro['idOrcamentos']."</td>";
        $linha.= "<td>".$registro['categoria']."</td>";
		$linha.= "<td>".$registro['servico']."</td>";
        $linha.= "<td>".$registro['nomepre']."</td>";
		$linha.= "<td>".$registro['nomecli']."</td>";
		$linha.="<td>".date("d/m/y",strtotime($registro['data_expiracao']))."</td>";
		...
		<tr>
		<td align="rigth"> <h3>Todos orcamentos no Período Selecionado :</h3></td>
		</tr>
		</table>
